            ...
            steps {
                ...
                dir("${APP_DIR}") {
                    ...
                    docker run --rm \
                        ...
                        docker/compose:latest docker compose down || true

                    docker run --rm \
                        -v /var/run/docker.sock:/var/run/docker.sock \
                        -v ${WORKSPACE}/app:/app -w /app \
                        docker/compose:latest docker compose up -d
                    """
                }
            }
        }
        ...
        stage('Verify Monitoring Services') {
            steps {
                ...
                dir("${APP_DIR}") {
                    ...
                    docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v ${WORKSPACE}/app:/app -w /app docker/compose:latest docker compose ps
                    '''
                }
            }
        }
